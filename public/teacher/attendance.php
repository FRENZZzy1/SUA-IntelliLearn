...
$selectedOffering = filter_input(INPUT_GET, 'offering_id', FILTER_VALIDATE_INT) ?: 0;
...
$late = (int)($overall['late_count'] ?? 0);
$excused = (int)($overall['excused_count'] ?? 0);
$attendanceRate = $totalRecords ? round((($present + $late) / $totalRecords) * 100, 1) : 0;

// Per-class analytics.
$classStats = [];
foreach ($classes as $class) {
    $stmt = $pdo->prepare("SELECT COUNT(*) total, SUM(status='Present') present, SUM(status='Absent') absent, SUM(status='Late') late, SUM(status='Excused') excused
        FROM attendance WHERE offering_id = ?");
    ...
    $classStats[$class['offering_id']] = [
        ...
        'absent' => (int)($stat['absent'] ?? 0),
        'late' => (int)($stat['late'] ?? 0),
        'excused' => (int)($stat['excused'] ?? 0),
        'rate' => $total ? round(((($stat['present'] ?? 0) + ($stat['late'] ?? 0)) / $total) * 100, 1) : 0,
    ];
}

// Students needing attention: below 75% attendance in the teacher's classes.
$atRiskStmt = $pdo->prepare("SELECT st.student_id, CONCAT(st.firstname, ' ', st.lastname) student_name,
    sec.section_name, s.subject_name,
    ...
    HAVING total > 0 AND ((present + late) / total) < 0.75
    ORDER BY ((present + late) / total) ASC, absent DESC LIMIT 10");
$atRiskStmt->execute([$teacherId, $schoolYearId, $schoolYearId]);
$atRisk = $atRiskStmt->fetchAll();

// Last 7 attendance dates for the trend chart.
$trendStmt = $pdo->prepare("SELECT a.attendance_date,
    ROUND(100 * SUM(a.status IN ('Present','Late')) / COUNT(*), 1) rate
...
<div class="att-grid">
<div class="att-card"><div class="num"><?= $attendanceRate ?>%</div><span class="label">Overall Attendance</span></div>
<div class="att-card"><div class="num"><?= $present ?></div><span class="label">Present</span></div>
<div class="att-card"><div class="num"><?= $absent ?></div><span class="label">Absent</span></div>
<div class="att-card"><div class="num"><?= $late ?></div><span class="label">Late</span></div>
<div class="att-card"><div class="num"><?= $excused ?></div><span class="label">Excused</span></div>
</div>

<div class="att-layout">
<section class="att-panel"><h2>My Classes</h2><div class="class-list">
<?php if (!$classes): ?><div class="empty">No active classes assigned.</div><?php endif; ?>
<?php foreach ($classes as $class): $cs=$classStats[$class['offering_id']]; ?>
<div class="class-row"><div><div class="class-name"><?= htmlspecialchars($class['subject_name']) ?></div><div class="class-meta">Grade <?= (int)$class['grade_level'] ?> · <?= htmlspecialchars($class['section_name']) ?></div></div><div><span class="rate"><?= $cs['rate'] ?>%</span> <a class="btn" href="class_overview.php?subject_id=<?= (int)$class['subject_id'] ?>&section_id=<?= (int)$class['section_id'] ?>&view=attendance&date=<?= htmlspecialchars($selectedDate) ?>"><i class="fas fa-clipboard-check"></i> Manage</a></div></div>
<?php endforeach; ?></div></section>

<section class="att-panel"><h2>Attendance Trend</h2>
<?php if (!$trend): ?><div class="empty">Attendance trends will appear after records are saved.</div><?php else: ?><div class="trend"><?php foreach($trend as $t): ?><div class="bar-wrap"><span class="bar-value"><?= htmlspecialchars($t['rate']) ?>%</span><div class="bar" style="height:<?= max(3,min(100,(float)$t['rate'])) ?>%"></div><span class="bar-label"><?= date('M j',strtotime($t['attendance_date'])) ?></span></div><?php endforeach; ?></div><?php endif; ?>
</section>
</div>

<section class="att-panel"><h2><i class="fas fa-triangle-exclamation"></i> Students Needing Attention</h2>
<?php if (!$atRisk): ?><div class="empty">No students currently fall below the 75% attendance threshold.</div><?php else: ?><div style="overflow-x:auto"><table class="risk-table"><thead><tr><th>Student</th><th>Class</th><th>Attendance</th><th>Absent</th><th>Status</th></tr></thead><tbody>
<?php foreach($atRisk as $r): $rate=round(((int)$r['present']+(int)$r['late'])/(int)$r['total']*100,1); ?><tr><td><?= htmlspecialchars($r['student_name']) ?></td><td><?= htmlspecialchars($r['subject_name'].' · '.$r['section_name']) ?></td><td><?= $rate ?>%</td><td><?= (int)$r['absent'] ?></td><td class="risk">At Risk</td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</section>
</div></main></body></html>