...
<main class="main-content" id="dashMain">
    ...
    <div class="qg-header">
        ...
        <p class="qg-subtitle">Describe a topic and let AI draft the quiz — review and edit everything before it goes live.</p>
    </div>
                ...
                <form id="qgGenerateForm">
                    <div class="qg-form-grid">
                            ...
                            <select id="qgOffering" name="offering_id" required>
                                ...
                                <?php foreach ($teacherOfferings as $o): ?>
                                    <option value="<?= (int) $o['offering_id'] ?>">
                                        <?= htmlspecialchars($o['subject_name']) ?> — Grade <?= (int) $o['grade_level'] ?> <?= htmlspecialchars($o['section_name']) ?> (<?= htmlspecialchars($o['quarter']) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="qg-field">
                            ...
                            <input type="number" id="qgNumItems" name="num_items" min="1" max="50" value="10" required>
                        </div>
                        ...
                        <div class="qg-field span-2">
                            <label for="qgTopic">Topic <span class="hint">— describe what the quiz should cover</span></label>
                            <textarea id="qgTopic" name="topic" required placeholder="e.g. Cell structure and function: organelles, their roles, plant vs. animal cell differences"></textarea>
                        </div>

                        <div class="qg-field span-2">
                            <label for="qgTitleInput">Quiz Title <span class="hint">— optional, AI will suggest one if left blank</span></label>
                            <input type="text" id="qgTitleInput" name="title" placeholder="e.g. Quarter 2 Quiz: Cell Biology">
                        </div>
                    ...
                    <?php foreach ($recentQuizzes as $rq): ?>
                        <div class="qg-recent-item">
                            ...
                            <span class="qg-recent-meta">
                                ...
                                <span class="qg-status-tag <?= htmlspecialchars($rq['status']) ?>"><?= htmlspecialchars($rq['status']) ?></span>
                            </span>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
